...
def normalize(data, mean, std):
    ...
    normalized_data.append(np.full(shape = len(data), fill_value = 1, dtype =np.int))
    normalized_data = np.vstack(normalized_data).T
...
def find_e_batch(X, Omega, y, s, l_variable, w):
    w_prime = w
    w_prime[len(w_prime)-1] = 0
    
    right_side = (X.T @ (y - s)) + 2*l_variable*w_prime
   
    left_side = (X.T@Omega @ X) + 2*l_variable
    e = np.linalg.inv(left_side) 
    e = e @ right_side
    return e
...
def grad_descent_batch(iteration, lambda_value):
    ...
    for i in range(iteration):
        ...
        e = find_e_batch(X, Omega, y, s, lambda_array, w)
        w = w + e
...
def visualize_iteration():
    number_of_iterations = np.arange(10)
    w_set = []
    accuracy_results = []
    for iteration in number_of_iterations:
        print(iteration)
        print("waiting...")
        w = grad_descent_batch(iteration, 0)
        w_set += [w]
...
def visualize_lambda():
    ...
    for lambda_value in lambda_values:
        ...
        print("waiting...")
        w = grad_descent_batch(iteration, lambda_value)
        w_set += [w]
        accuracy_results += [evaluate(validation_labels, predict(validation_data, w))]
...
def kaggle():
    iteration = 9
    w = grad_descent_batch(iteration, 0.01)
